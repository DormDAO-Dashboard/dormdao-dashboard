...
function MemberCard({ m, schoolName }: { m: FilteredMember; schoolName: string }) {
  ...
  return (
    <Link href={`/users/${m.id}`}
      ...
      {/* Bio */}
      {m.bio && (
        <p className="text-xs text-gray-700 dark:text-gray-400 line-clamp-2 leading-relaxed">{m.bio}</p>
      )}
...
export function SchoolMembers({
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-xs text-gray-700 dark:text-gray-400">{members.length} member{members.length !== 1 ? "s" : ""}</p>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {members.map((m) => (
