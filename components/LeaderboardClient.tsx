...
import { schoolDisplayName, schoolShortName, getQuarterLabel } from "@/lib/schoolData";
...
function QuarterlyTable({ schools, userSlug }: { schools: SchoolRow[]; userSlug: string | null }) {
  ...
  return (
    <table className="w-full text-xs" style={{ height: '100%' }}>
      <thead className="sticky top-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm">
        <tr className="border-b border-gray-200 dark:border-gray-800">
          <th className="px-3 py-2 whitespace-nowrap text-left text-[10px] uppercase tracking-wide text-gray-500">#</th>
...
function SeasonTable({ schools, userSlug }: { schools: SchoolRow[]; userSlug: string | null }) {
  ...
  return (
    <table className="w-full text-xs" style={{ height: '100%' }}>
      <thead className="sticky top-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm">
        <tr className="border-b border-gray-200 dark:border-gray-800">
          <th className={th("rank", "text-left")} onClick={() => toggle("rank")}>
        ...
        {sorted.map((s, i) => {
          ...
          return (
            ...
            >
              ...
              <td className="px-2 py-2 text-right font-mono text-gray-700 dark:text-gray-300 text-[10px] tabular-nums whitespace-nowrap">{formatNav(s.nav)}</td>
              <td className="px-2 py-2 text-right whitespace-nowrap"><ReturnCell value={s.usdReturn} /></td>
              <td className="px-2 py-2 text-right whitespace-nowrap"><ReturnCell value={s.ethReturn} /></td>
              <td className="px-2 py-2 text-right font-mono text-gray-600 dark:text-gray-400 text-[10px] tabular-nums whitespace-nowrap">
...
function AllTimeTable({ schools, userSlug }: { schools: SchoolRow[]; userSlug: string | null }) {
  ...
  return (
    <table className="w-full text-xs" style={{ height: '100%' }}>
      <thead className="sticky top-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm">
        <tr className="border-b border-gray-200 dark:border-gray-800">
          <th className={cn(th, "text-left")} onClick={() => toggle("rank")}>
