...
function MemberRow({ m }: { m: MemberProfile }) {
  ...
  return (
    <Link href={`/users/${m.id}`}
      ...
      {m.avatar_url && !avatarFailed ? (
        <Image src={m.avatar_url} width={36} height={36} alt={m.display_name}
          className="rounded-lg shrink-0 object-cover" unoptimized
          onError={() => setAvatarFailed(true)} />
      ) : (
        <div className={`w-9 h-9 rounded-lg shrink-0 flex items-center justify-center text-white text-sm font-semibold ${avatarColor(m.id)}`}>
      ...
      {hasSocial && (
        <div className="flex items-center gap-3 ml-auto pl-2 shrink-0">
          {m.twitter && (
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 divide-y divide-gray-200 dark:divide-gray-800 overflow-hidden">
