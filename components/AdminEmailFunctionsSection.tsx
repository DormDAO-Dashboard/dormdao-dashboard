        ...
        <div className="divide-y divide-gray-200 dark:divide-gray-800">
          {templates.map((t) => (
            <div key={t.key} className="flex items-center gap-4 px-5 py-4">
              <Mail className="w-4 h-4 text-gray-700 dark:text-gray-400 shrink-0" />
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => setViewTarget(t)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:border-gray-300 dark:hover:border-gray-600 transition-colors text-xs font-medium">
          ...
          {templates.length === 0 && (
            <div className="px-5 py-10 text-center text-sm text-gray-700 dark:text-gray-400">No email types found.</div>
          )}
        </div>
      )}
      ...
      {viewTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg max-h-[85vh] overflow-y-auto bg-white dark:bg-[#111] rounded-xl border border-gray-200 dark:border-gray-800 shadow-2xl p-6">
            ...
            <div className="rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
              ...
              <div className="bg-white px-5 py-5">
                <p className="text-[11px] text-gray-700 dark:text-gray-400 uppercase tracking-wide mb-3">
                  Subject:{" "}
                  <span className="normal-case text-gray-700 dark:text-gray-400 font-medium">
                    {fillSample(fieldValue(viewTarget, "subject"), viewTarget.sampleVars)}
                  </span>
                </p>
                <h2 className="text-lg font-bold text-gray-900 dark:text-gray-400 mb-3">
                  ...
                  return (
                    <div key={f.key} className="mb-3">
                      {viewExtraFields.length > 1 && (
                        <p className="text-[10px] text-gray-700 dark:text-gray-400 uppercase tracking-wide mb-1">{f.label}</p>
                      )}
                      {text.split(/\n{2,}/).map((p, i) => (
                        <p key={i} className="text-sm text-gray-700 dark:text-gray-400 leading-relaxed mt-1 first:mt-0">{p}</p>
                      ))}
                    </div>
                  );
                })}
            ...
            <p className="text-[11px] text-gray-700 dark:text-gray-400 mt-3">
