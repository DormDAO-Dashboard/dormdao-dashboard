    ...
    <a href={url} target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60 p-3 hover:border-gray-300 dark:hover:border-gray-600 transition-colors">
      <div className="flex gap-3">
        ...
        <div className="min-w-0">
          ...
          {data.description && <div className="text-xs text-gray-700 dark:text-gray-400 line-clamp-2 mt-0.5">{data.description}</div>}
        </div>
      </div>
    </a>
  );
...
export function NoteCard({ note, currentUserId, adminSecret, onDelete, onUpvote }: NoteCardProps) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {note.token_ticker && (
            <Link
              ...
              className="text-xs font-mono bg-gray-800 text-gray-700 dark:text-gray-400 px-2 py-0.5 rounded hover:text-gray-900 dark:hover:text-white transition-colors"
            >
              ${note.token_ticker}
            </Link>
          )}
          ...
          {note.school && (
            <span className="text-xs text-gray-700 dark:text-gray-400">{schoolDisplayName(note.school)}</span>
          )}
        </div>
        {canDelete && (
          <button
            ...
            disabled={deleting}
            className="text-gray-700 dark:text-gray-400 hover:text-danger transition-colors p-1 disabled:opacity-40"
      ...
      {note.content && <p className="text-sm text-gray-700 dark:text-gray-400 leading-relaxed">{note.content}</p>}
      {note.url && <LinkPreviewCard url={note.url} />}

      <div className="flex items-center justify-between">
        <div className="text-xs text-gray-700 dark:text-gray-400">
          <span className="font-medium text-gray-700 dark:text-gray-400">{note.author_name}</span>
          {" · "}
        ...
        <button
          ...
          className={cn(
            ...
            voted
              ...
              : "text-gray-700 dark:text-gray-400 hover:text-primary hover:bg-primary/10"
          )}
        >
