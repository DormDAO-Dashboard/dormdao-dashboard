    ...
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 py-20 text-center">
          <EyeOff className="w-8 h-8 text-gray-700 dark:text-gray-400 mx-auto mb-4" />
          <p className="text-gray-700 dark:text-gray-400 text-sm">This profile is private.</p>
          {!viewer && (
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex items-center justify-between mb-8">
        <Link href="/schools"
          className="inline-flex items-center gap-1.5 text-sm text-gray-700 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
          <ArrowLeft className="w-4 h-4" /> Back to Schools
        </Link>
        {isOwner && (
          <Link href="/profile"
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 text-sm text-gray-700 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
        ...
        <div className="flex items-start gap-5">
          {profile.avatar_url ? (
            ...
            <div className="w-[72px] h-[72px] rounded-xl bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center shrink-0">
              <User className="w-7 h-7 text-gray-700 dark:text-gray-400" />
            </div>
          )}
          ...
          <div className="flex-1 min-w-0">
            ...
            {canShow("school") && profile.school === MAIN_DAO_VOTER && (
              <Link href="/main-dao"
                className="flex items-center gap-1.5 mt-1 text-sm text-gray-700 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition-colors w-fit">
                {MAIN_DAO_VOTER}
              </Link>
            )}
            ...
            <div className="flex flex-wrap gap-3 mt-1.5">
              ...
              {canShow("major") && profile.major && (
                <span className="text-xs text-gray-700 dark:text-gray-400">{profile.major}</span>
              )}
            </div>

            {memberSince && (
              <p className="text-xs text-gray-700 dark:text-gray-400 mt-0.5">Joined {memberSince}</p>
            )}

            {canShow("bio") && profile.bio && (
              <p className="text-sm text-gray-700 dark:text-gray-300 mt-3 leading-relaxed">{profile.bio}</p>
            )}
          </div>
        </div>
      ...
      {isOwner && (
        <div className="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-400 px-1 mb-6">
          {profile.is_public
      ...
      <section>
        ...
        {notes && notes.length > 0 ? (
          ...
          <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 py-10 text-center text-sm text-gray-700 dark:text-gray-400">
